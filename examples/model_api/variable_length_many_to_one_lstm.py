...
import mithril
from mithril import TBD, JaxBackend
...
compiled_model = mithril.compile(
    model=train_ctx,
    backend=backend,
    static_keys=train_static_inputs,
)

# Randomize the trainable inputs.
params: [str, backend.ArrayType] = compiled_model.randomize_params()  # type: ignore
...
compiled_test_model = mithril.compile(
    model=test_model,
    backend=backend,
    static_keys={key: TBD for key in test_static_inputs},
)
