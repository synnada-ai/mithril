...
import mithril
from mithril import TBD, IOKey, TorchBackend
...
logical_model += age_head(input=backbone.canonical_output, output=IOKey("age"))
logical_model += gender_head(input=backbone.canonical_output)
logical_model += SVM_Model(
    input=gender_head.output,
    output=IOKey("gender"),
    decision_output=IOKey("pred_gender"),
)
...
backend = TorchBackend(precision=32, device="mps")

# Compile the model with given non-trainable keys
compiled_model = mithril.compile(
    model=train_model,
    backend=backend,
    static_keys={"input": TBD, "age_target": TBD, "gender_target": TBD},
