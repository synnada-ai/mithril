...
def process_input(audio:np.ndarray)->np.ndarray:
    ...
    normalized_mel = 2 * (standardized_mel - mel_min) / (mel_max - mel_min) - 1  # Normalize to [-1, 1]
...
def run_inference(file_path:str,backend:str)->str:
    ...
    compiled_model = ml.compile(
         whisper, backend_obj, data_keys={"input","decoder_input_ids"}, jit=False, use_short_namings=False
     )
    params = get_weights(backend_obj)
    input_features = backend_obj.array(process_input(sample)) #Generate mel-spectograms for sample input
    ...
    while token != 50257: #Start token for whisper
        outputs = compiled_model.evaluate(params, data={"input":input_features,"decoder_input_ids":decoder_input_ids})
