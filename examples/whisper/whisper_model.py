...
def whisper_attention(name:str,input_dim:int,num_heads:int,is_causal:bool=False,cross_attention:bool=False):
    ...
    model += ScaledDotProduct(is_causal=is_causal)(query=tq, key=tk, value=tv, output="sdp_out")
...
def encoder_block(num_layers:int,input_dim:int,num_heads:int,ffn_dim:int):
    ...
    for idx in range(num_layers):
        block = Model(name="{}".format(idx))
        block += LayerNorm(name="self_attn_layer_norm")("layer_in",output="layer_norm_out")
...
def decoder_block(num_layers:int,input_dim:int,num_heads:int,ffn_dim:int):
    ...
    for idx in range(num_layers):
        block = Model(name="{}".format(idx))
        block += LayerNorm(name="self_attn_layer_norm")(input="layer_in",output="layer_norm_out")
        block += whisper_attention('self_attn',input_dim,num_heads,is_causal=True) #Self attention between decoder ids
        ...
        block += whisper_attention('encoder_attn',input_dim,num_heads,is_causal=False,cross_attention=True)(input=block.cout,xa="encoder_hidden_states")
...
def whisper_encoder(num_layers:int,input_dim:int,num_heads:int,ffn_dim:int):
    ...
    model += Convolution1D(out_channels=input_dim,kernel_size=3,padding=1, name="conv1")(input="input",output="conv1_out")
    model += Gelu()(input="conv1_out",output="gelu1_out")
    model += Convolution1D(out_channels=input_dim,kernel_size=3,stride=2,padding=1, name="conv2")(input="gelu1_out",output="conv2_out")
    ...
    model += Embedding(name="embed_positions", num_embeddings=1500, dim=input_dim)(input="embedding_in",
        output="pos_out") #Sinusiodal positional embeddings 
...
def whisper_decoder(num_layers:int,input_dim:int,num_heads:int,ffn_dim:int):
    ...
    model += Embedding(name="embed_tokens",num_embeddings=51865,dim=input_dim)(input="decoder_input_ids",output="embedded_tokens")
    model += Size(dim=1)("embedded_tokens") #Decoder id token embeddings
    model += Arange()
    model += Embedding(name="embed_positions",num_embeddings=448,dim=input_dim)(output="embedded_positions") #Positional embedding for decoder ids
...
def whisper_model(num_layers:int,input_dim:int,num_heads:int,ffn_dim:int,eps:float=1e-5):
    model = Model(name="model")
    model += whisper_encoder(num_layers, input_dim,num_heads, ffn_dim)(input="input",encoder_hidden_states="encoder_hidden_states")
    decoder_model =whisper_decoder(num_layers,input_dim,num_heads,ffn_dim)
    model += decoder_model(encoder_hidden_states="encoder_hidden_states",decoder_input_ids="decoder_input_ids")
    whisper = Model()
    whisper += model(input="input",decoder_input_ids="decoder_input_ids")
    whisper += Linear(51865,name="proj_out",use_bias=False) #Mapping decoder output to vocabulary tokens
    return whisper
