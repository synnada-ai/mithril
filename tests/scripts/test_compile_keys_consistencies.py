...
from mithril.models import Model, Linear, Multiply
from mithril import TorchBackend, IOKey
...
def test_dollar_sign_str():
    ...
    kwargs: dict
    for key in ["constant_keys", "data_keys", "discard_keys", "jacobian_keys", "trainable_keys", "shapes"]:
        ...
        assert (
            str(err_info.value) == 
                f"'Given key: $1 is not valid. Unnamed keys in logical model "
...
def test_connection_not_found():
    ...
    for key in ["constant_keys", "data_keys", "discard_keys", "jacobian_keys", "trainable_keys", "shapes"]:
        ...
        assert (
            str(err_info.value) == 
                f"'Given connection not found: {mult_model.left}'"
        )
...
def test_string_not_found():
    ...
    for key in ["constant_keys", "data_keys", "discard_keys", "jacobian_keys", "trainable_keys", "shapes"]:
        ...
        assert (
            str(err_info.value) == 
                f"'Given key: left is not found in the logical model.'"
        )
...
def test_reset_static_data():
    ...
    for key in ["constant_keys", "data_keys"]:
        ...
        assert (
            str(err_info.value) == 
                f"Statically given key: input has been already "
...
def test_check_keys_disjoint_sets():
    ...
    backend = TorchBackend()
    with pytest.raises(ValueError) as err_info:
        ml_compile(model, backend, constant_keys={lin_model.input: backend.ones(1, 1)}, data_keys={"input"})
    assert (
        str(err_info.value) == 
    ...
    with pytest.raises(ValueError) as err_info:
        ml_compile(model, backend, data_keys={"input"}, trainable_keys={lin_model.input})
    assert (
        str(err_info.value) == 
                "Constant, data, trainable and discard keys must be disjoint sets. "
                "Common keys (in physical domain) in at least 2 different sets: input."
...
def test_discard_keys_input_and_outputs_only():
    ...
    backend = TorchBackend()
    kwargs = {
        "discard_keys": {lin_model.output, "output"}
    }
    with pytest.raises(KeyError) as err_info:
