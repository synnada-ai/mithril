...
def test_two_conns():
    ...
    model_1 = Model()
    model_1 |= Linear(dimension=2).connect(input="input", weight="weight", bias="bias")
    add_1 = model_1.input + model_1.bias  # type: ignore
    model_1 |= Mean().connect(input=add_1, output=IOKey(name="output"))

    # Create with extend.
    model_2 = Model()
    model_2 |= Linear(dimension=2).connect(input="input", weight="weight", bias="bias")
    model_2 |= (add_2 := Add()).connect(left=model_2.input, right=model_2.bias)  # type: ignore
    model_2 |= Mean().connect(input=add_2.output, output=IOKey(name="output"))
...
def test_conn_template():
    ...
    model_1 |= Add().connect(left=add_1, right=add_2, output=IOKey(name="output"))
    ...
    add_4 = Add()
    model_2 |= Linear(dimension=2).connect(input="input", weight="weight", bias="bias")
    model_2 |= add_3.connect(left=model_2.input, right=model_2.bias)  # type: ignore
    model_2 |= add_4.connect(left=add_3.output, right=model_2.bias)  # type: ignore
    model_2 |= Add().connect(
        left=add_3.output, right=add_4.output, output=IOKey(name="output")
    )
...
def test_template_template():
    ...
    # Create with shortcut.
    model_1 = Model()
    model_1 |= (lin_1 := Linear(dimension=2)).connect(
        input="input_1", weight="w_1", bias="b_1"
    )
    model_1 |= (tensor := ToTensor()).connect(input=2.0)
    model_1 |= (lin_2 := Linear(dimension=2)).connect(
        input="input_2", weight="w_2", bias="b_2"
    )
    ...
    add_3 = add_1 + add_2
    model_1 |= Add().connect(
        left=tensor.output, right=add_3, output=IOKey(name="output")
    )

    # Create with extend.
    model_2 = Model()
    model_2 |= (lin_3 := Linear(dimension=2)).connect(
        input="input_1", weight="w_1", bias="b_1"
    )
    model_2 |= (tensor := ToTensor()).connect(input=2.0)
    model_2 |= (lin_4 := Linear(dimension=2)).connect(
        input="input_2", weight="w_2", bias="b_2"
    )
    model_2 |= (add_4 := Add()).connect(left=lin_3.input, right=lin_3.bias)
    model_2 |= (add_5 := Add()).connect(left=lin_4.input, right=lin_4.bias)
    model_2 |= (add_6 := Add()).connect(left=add_4.output, right=add_5.output)
    model_2 |= Add().connect(
...
def test_shape_reshape():
    ...
    # Create with shortcut.
    model_1 = Model()
    model_1 |= (lin_1 := Linear(dimension=1)).connect(
        input="input_1", weight="w_1", bias="b_1"
    )
    model_1 |= (lin_2 := Linear(dimension=2)).connect(
        input="input_2", weight="w_2", bias="b_2"
    ...
    model_1 |= Add().connect(
        left=lin_1.output, right=reshaped, output=IOKey(name="output")
    )

    # Create with extend.
    model_2 = Model()
    model_2 |= (lin_3 := Linear(dimension=1)).connect(
        input="input_1", weight="w_1", bias="b_1"
    )
    model_2 |= (lin_4 := Linear(dimension=2)).connect(
        input="input_2", weight="w_2", bias="b_2"
    )
    model_2 |= (shp_model := Shape()).connect(input=lin_3.input)
    model_2 |= (re_shp := Reshape()).connect(input=lin_4.output, shape=shp_model.output)
    model_2 |= Add().connect(
...
def test_slice_item():
    ...
    item = shp[1].tensor()
    slc = shp[:].tensor()
    model_1 |= Add().connect(left=item, right=slc, output=IOKey(name="output"))
    ...
    model_2 |= (shp_model := Shape()).connect(input=lin_3.input)
    model_2 |= (item_model := Indexer()).connect(input=shp_model.output, index=1)
    model_2 |= (tensor_1 := ToTensor()).connect(input=item_model.output)
    model_2 |= (slc_1 := Slice()).connect(start=None, stop=None, step=None)
    model_2 |= (slice_model := Indexer()).connect(
        input=shp_model.output, index=slc_1.output
    )
    model_2 |= (tensor_2 := ToTensor()).connect(input=slice_model.output)
    model_2 |= Add().connect(
...
def test_right_add():
    ...
    model_2 |= Mean().connect(input=add_2, output=IOKey(name="output"))

    # Create first model with extend.
    model_3 = Model()
    model_3 |= Linear(dimension=2).connect(input="input", weight="weight", bias="bias")
    model_3 |= (add_3 := Add()).connect(left=model_3.input, right=Tensor(2.0))  # type: ignore
    model_3 |= Mean().connect(input=add_3.output, output=IOKey(name="output"))

    # Create second model with extend.
    model_4 = Model()
    model_4 |= Linear(dimension=2).connect(input="input", weight="weight", bias="bias")
    model_4 |= (add_4 := Add()).connect(left=Tensor(2.0), right=model_4.input)  # type: ignore
    model_4 |= Mean().connect(input=add_4.output, output=IOKey(name="output"))
...
def test_right_pow():
    ...
    model_1 |= Mean().connect(input=pow_1, output=IOKey(name="output"))

    # Create with shortcut using right add.
    model_2 = Model()
    model_2 |= Linear(dimension=2).connect(input="input", weight="weight", bias="bias")
    pow_2 = Tensor(2.0) ** model_2.input  # type: ignore
    model_2 |= Mean().connect(input=pow_2, output=IOKey(name="output"))

    # Create first model with extend.
    model_3 = Model()
    model_3 |= Linear(dimension=2).connect(input="input", weight="weight", bias="bias")
    model_3 |= (pow_3 := Power()).connect(base=model_3.input, exponent=Tensor(2.0))  # type: ignore
    model_3 |= Mean().connect(input=pow_3.output, output=IOKey(name="output"))

    # Create second model with extend.
    model_4 = Model()
    model_4 |= Linear(dimension=2).connect(input="input", weight="weight", bias="bias")
    model_4 |= (pow_4 := Power()).connect(base=Tensor(2.0), exponent=model_4.input)  # type: ignore
    model_4 |= Mean().connect(input=pow_4.output, output=IOKey(name="output"))
...
def test_multiple_op_order_1():
    ...
    model = Model()
    model |= Linear(dimension=2).connect(input="input", weight="weight", bias="bias")
    model |= (mul := Multiply()).connect(left=2.0, right="input")
    model |= (add := Add()).connect(left="input", right=mul.output)
    model |= Mean().connect(input=add.output, output=IOKey(name="output"))
...
def test_multiple_op_order_2():
    ...
    model |= Buffer().connect(input=op_out, output=IOKey(name="output"))

    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (matmul := MatrixMultiply()).connect(left="input", right="input")
    model2 |= (m := Multiply()).connect(left=5.0, right="input")
    model2 |= (add := Add()).connect(left=matmul.output, right=m.output)
    model2 |= Buffer().connect(input=add.output, output=IOKey(name="output"))
...
def test_sequence_slice_1():
    ...
    model |= Buffer().connect(input=output, output=IOKey(name="output"))
...
def test_mul():
    ...
    model1 = Model()
    model1 |= Buffer().connect(input="input")
    output = model1.input * Tensor(2)  # type: ignore
    model1 |= Buffer().connect(input=output, output=IOKey(name="output"))

    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (mul := Multiply()).connect(left="input", right=Tensor(2))
    model2 |= Buffer().connect(input=mul.output, output=IOKey(name="output"))
...
def test_rmul():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (mul := Multiply()).connect(left=Tensor(2), right="input")
    model2 |= Buffer().connect(input=mul.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_div():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (div := Divide()).connect(numerator="input", denominator=Tensor(2))
    model2 |= Buffer().connect(input=div.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_rdiv():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (div := Divide()).connect(numerator=Tensor(2), denominator="input")
    model2 |= Buffer().connect(input=div.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_floor_div():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (div := FloorDivide()).connect(numerator="input", denominator=Tensor(2))
    model2 |= Buffer().connect(input=div.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_rfloor_div():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (div := FloorDivide()).connect(numerator=Tensor(2), denominator="input")
    model2 |= Buffer().connect(input=div.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
    pm = mithril.compile(
        model=model1, backend=backend, constant_keys=data, inference=True
...
def test_pow():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (div := Power()).connect(base="input", exponent=Tensor(2))
    model2 |= Buffer().connect(input=div.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_rpow():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (div := Power()).connect(base=Tensor(2), exponent="input")
    model2 |= Buffer().connect(input=div.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_absolute():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (abs := Absolute()).connect(input="input")
    model2 |= Buffer().connect(input=abs.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_exp():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (exp := Exponential()).connect(input="input")
    model2 |= Buffer().connect(input=exp.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_mean():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (mean := Mean()).connect(input="input")
    model2 |= Buffer().connect(input=mean.output, output=IOKey(name="output"))
...
def test_max():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (max := Max()).connect(input="input")
    model2 |= Buffer().connect(input=max.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, check_internals=False, inference=True)
...
def test_sum():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (sum := Sum()).connect(input="input")
    model2 |= Buffer().connect(input=sum.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, check_internals=False, inference=True)
...
def test_min():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (min := Min()).connect(input="input")
    model2 |= Buffer().connect(input=min.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, check_internals=False, inference=True)
...
def test_prod():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (prod := Prod()).connect(input="input")
    model2 |= Buffer().connect(input=prod.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, check_internals=False, inference=True)
...
def test_variance():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (var := Variance()).connect(input="input")
    model2 |= Buffer().connect(input=var.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, check_internals=False, inference=True)
...
def test_greater_than():
    ...
    model1 = Model()
    model1 |= Buffer().connect(input="input1")
    model1 |= Buffer().connect(input="input2")
    output = model1.input1 > model1.input2  # type: ignore
    model1 |= Buffer().connect(input=output, output=IOKey(name="output"))

    model2 = Model()
    model2 |= Buffer().connect(input="input1")
    model2 |= Buffer().connect(input="input2")
    model2 |= (var := Greater()).connect(left="input1", right="input2")
    model2 |= Buffer().connect(input=var.output, output=IOKey(name="output"))
...
def test_greater_equal():
    ...
    model2 |= Buffer().connect(input="input1")
    model2 |= Buffer().connect(input="input2")
    model2 |= (var := GreaterEqual()).connect(left="input1", right="input2")
    model2 |= Buffer().connect(input=var.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_less_than():
    ...
    model2 |= Buffer().connect(input="input1")
    model2 |= Buffer().connect(input="input2")
    model2 |= (var := Less()).connect(left="input1", right="input2")
    model2 |= Buffer().connect(input=var.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_less_equal():
    ...
    model2 |= Buffer().connect(input="input1")
    model2 |= Buffer().connect(input="input2")
    model2 |= (var := LessEqual()).connect(left="input1", right="input2")
    model2 |= Buffer().connect(input=var.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_equal():
    ...
    model2 |= Buffer().connect(input="input1")
    model2 |= Buffer().connect(input="input2")
    model2 |= (var := Equal()).connect(left="input1", right="input2")
    model2 |= Buffer().connect(input=var.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_not_equal():
    ...
    model2 |= Buffer().connect(input="input1")
    model2 |= Buffer().connect(input="input2")
    model2 |= (var := NotEqual()).connect(left="input1", right="input2")
    model2 |= Buffer().connect(input=var.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_not():
    ...
    model2 |= Buffer().connect(input="input2")
    model2 |= (var := NotEqual()).connect(left="input1", right="input2")
    model2 |= (lnot := LogicalNot()).connect(input=var.output)
    model2 |= Buffer().connect(input=lnot.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_and():
    ...
    model2 |= Buffer().connect(input="input1")
    model2 |= Buffer().connect(input="input2")
    model2 |= (g1 := Greater()).connect(left="input1", right=Tensor(0))
    model2 |= (g2 := Greater()).connect(left="input2", right=Tensor(3))
    model2 |= (land := LogicalAnd()).connect(left=g1.output, right=g2.output)
    model2 |= Buffer().connect(input=land.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_or():
    ...
    model2 |= (g1 := Greater()).connect(left="input1", right=Tensor(0))
    model2 |= (g2 := Greater()).connect(left="input2", right=Tensor(3))
    model2 |= (lor := LogicalOr()).connect(left=g1.output, right=g2.output)
    model2 |= Buffer().connect(input=lor.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_xor():
    ...
    model2 |= (g1 := Greater()).connect(left="input1", right=Tensor(0))
    model2 |= (g2 := Greater()).connect(left="input2", right=Tensor(3))
    model2 |= (lor := LogicalXOr()).connect(left=g1.output, right=g2.output)
    model2 |= Buffer().connect(input=lor.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_xor2():
    ...
    model2 |= Buffer().connect(input="input1")
    model2 |= Buffer().connect(input="input2")
    model2 |= (g2 := Greater()).connect(left="input2", right=3)
    model2 |= (lor := LogicalXOr()).connect(
...
def test_lshift_1():
    ...
    model1 = Model()
    model1 |= Buffer().connect(input="input")
    model1 |= Buffer().connect(input="shift")
    output = model1.input << model1.shift  # type: ignore
    model1 |= Buffer().connect(input=output, output=IOKey(name="output"))

    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= Buffer().connect(input="shift")
    model2 |= (sl := ShiftLeft()).connect(input="input", shift="shift")
    model2 |= Buffer().connect(input=sl.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_lshift_2():
    ...
    model2 |= Buffer().connect(input="input")
    model2 |= Buffer().connect(input="shift")
    model2 |= (sl := ShiftLeft()).connect(input="input", shift=Tensor(2))
    model2 |= Buffer().connect(input=sl.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_lshift_3():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (sl := ShiftLeft()).connect(input=Tensor(2), shift="input")
    model2 |= Buffer().connect(input=sl.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_rshift_1():
    ...
    model2 |= Buffer().connect(input="input")
    model2 |= Buffer().connect(input="shift")
    model2 |= (sr := ShiftRight()).connect(input="input", shift="shift")
    model2 |= Buffer().connect(input=sr.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_rshift_2():
    ...
    model2 |= Buffer().connect(input="input")
    model2 |= Buffer().connect(input="shift")
    model2 |= (sl := ShiftRight()).connect(input="input", shift=Tensor(2))
    model2 |= Buffer().connect(input=sl.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_rshift_3():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (sl := ShiftRight()).connect(input=Tensor(2), shift="input")
    model2 |= Buffer().connect(input=sl.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_negate():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (negate := Negate()).connect(input="input")
    model2 |= Buffer().connect(input=negate.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_cast():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (cast := Cast(dtype=mithril.float16)).connect(input="input")
    model2 |= Buffer().connect(input=cast.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_sin():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (sin := Sine()).connect(input="input")
    model2 |= Buffer().connect(input=sin.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_cos():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (cos := Cosine()).connect(input="input")
    model2 |= Buffer().connect(input=cos.output, output=IOKey(name="output"))
    compare_models(model1, model2, backend, data, inference=True)
...
def test_use_submodel_conn_1():
    ...
    modelsub = Model()
    modelsub |= Buffer().connect(input="input1", output=IOKey(name="output"))
    x = (modelsub.input1 + Tensor(3)) / Tensor(2)  # type: ignore
    x += Tensor(3)

    model1 = Model()
    model1 |= modelsub.connect(input1="input1")
    model1 |= Buffer().connect(input=x, output=IOKey(name="output"))

    modelsub2 = Model()
    modelsub2 |= Buffer().connect(input="input1", output=IOKey(name="output"))

    model2 = Model()
    model2 |= modelsub2.connect(input1="input1")
    model2 |= (add := Add()).connect(left="input1", right=Tensor(3))
    model2 |= (div := Divide()).connect(numerator=add.output, denominator=Tensor(2))
    model2 |= (add2 := Add()).connect(left=div.output, right=Tensor(3))
    model2 |= Buffer().connect(input=add2.output, output=IOKey(name="output"))
...
def test_use_multiple_times():
    ...
    model1 = Model()
    model1 |= Buffer().connect(input="input1", output=IOKey(name="output"))
    x = (model1.input1 + Tensor(3)) / Tensor(2)  # type: ignore
    model1 |= Buffer().connect(input=x, output=IOKey(name="output1"))
    model1 |= Relu().connect(input=x, output=IOKey(name="output2"))

    model2 = Model()
    model2 |= Buffer().connect(input="input1", output=IOKey(name="output"))
    model2 |= (add := Add()).connect(left="input1", right=Tensor(3))
    model2 |= (div := Divide()).connect(numerator=add.output, denominator=Tensor(2))
    model2 |= Buffer().connect(input=div.output, output=IOKey(name="output1"))
    model2 |= Relu().connect(input=div.output, output=IOKey(name="output2"))
...
def test_index_multiple_slice_1():
    ...
    model1 |= buffer_model_1.connect(input="input")
    model1 |= slice_model_1.connect(start=2, stop=3, step=None)
    model1 |= slice_model_2.connect(start=4, stop=6, step=None)
    model1 |= to_tuple_model.connect(
        input1=slice_model_1.output, input2=slice_model_2.output
    )
    model1 |= item_model.connect(
        input=buffer_model_1.output, index=to_tuple_model.output
    )
    model1 |= buffer_model_2.connect(input=item_model.output, output=IOKey("output"))

    model2 = Model()
    buffer_model_1 = Buffer()
    model2 |= buffer_model_1.connect(input=IOKey("input", type=Tensor))
    conn = buffer_model_1.output[2:3, 4:6]
    buffer_model_2 = Buffer()
    buffer_model_2.set_types(input=Tensor)
    model2 |= buffer_model_2.connect(input=conn, output=IOKey("output"))
...
def test_index_multiple_slice_2():
    ...
    model2 |= buffer_model_3.connect(input=conn, output=IOKey("output"))
    check_logical_models(model1, model2)
...
def test_tensor_item_with_ellipsis_at_beginning():
    ...
    model |= buff_model.connect(input=input[..., 3], output="output")
...
def test_tensor_item_with_ellipsis_in_middle():
    ...
    buff_model = Buffer()
    buff_model.set_types(input=Tensor)
    model |= buff_model.connect(input=input[0, ..., 3], output="output")
...
def test_tranpose_1():
    ...
    model |= Buffer().connect(input=result, output="output")
...
def test_split_compare_with_explicit():
    ...
    model2 = Model()
    model2 |= Buffer().connect(input="input")
    model2 |= (split := Split(split_size=2, axis=1)).connect(input="input")
    model2 |= Buffer().connect(input=split.output, output=IOKey(name="output"))
...
def test_immediate_values_with_extend_template_and_regular_case():
    ...
    model = Model()
    model |= (buff := Buffer()).connect(input="input")
    conn = buff.output[2]
    model |= Buffer().connect(input=conn, output="output")

    big_model_1 = Model()
    big_model_1 |= model.connect(input="input", output="output")

    # Regular case.
    model = Model()
    model |= (buff := Buffer()).connect(input="input")
    model += (item := Indexer()).connect(index=2)
    model |= Buffer().connect(input=item.output, output="output")

    big_model_2 = Model()
    big_model_2 |= model.connect(input="input", output="output")
...
def test_item():
    ...
    model2 |= ToTensor().connect(input=conn, output=IOKey("output"))
...
def test_tensor_item_with_slice():
    ...
    model1 |= Buffer().connect(input=output, output=IOKey("output"))

    model2 = Model()
    item_model = Indexer()
    slice_model = Slice()
    buffer = Buffer()
    model2 |= slice_model.connect(start=1, stop=2, step=None)
    model2 |= item_model.connect(input="input", index=slice_model.output)
    model2 |= buffer.connect(input=item_model.output, output=IOKey("output"))
...
def test_tensor_item_with_tuple_of_slice_and_int():
    ...
    model2 |= slice_model.connect(start=1, stop=2, step=None)
    model2 |= to_tuple_model.connect(input1=slice_model.output, input2=3)
    model2 |= item_model.connect(input="input", index=to_tuple_model.output)
    model2 |= buffer.connect(input=item_model.output, output=IOKey("output"))
...
def test_tensor_item_with_shape_dependent_slice():
    ...
    model2 |= shape_model.connect(input="input2")
    model2 |= scalar_item_model.connect(input=shape_model.output, index=1)
    model2 |= slice_model.connect(start=scalar_item_model.output, stop=None, step=None)
    model2 |= tensor_item_model.connect(input="input1", index=slice_model.output)
    model2 |= buffer.connect(input=tensor_item_model.output, output=IOKey("output"))
...
def test_tensor_item_with_tuple_of_shape_dependent_slices():
    ...
    model2 |= shape_model_1.connect(input="input2")
    model2 |= scalar_item_model_1.connect(input=shape_model_1.output, index=1)
    model2 |= slice_model_1.connect(
        start=scalar_item_model_1.output, stop=None, step=None
    )

    model2 |= shape_model_2.connect(input="input2")
    model2 |= scalar_item_model_2.connect(input=shape_model_2.output, index=0)
    model2 |= slice_model_2.connect(
        start=None, stop=scalar_item_model_2.output, step=None
    )

    model2 |= to_tuple_model.connect(
        input1=slice_model_1.output, input2=slice_model_2.output
    )
    model2 |= tensor_item_model.connect(input="input1", index=to_tuple_model.output)
    model2 |= buffer.connect(input=tensor_item_model.output, output=IOKey("output"))
