...
def test_linspace_1():
    ...
    for backend in all_backend:
        if not backend.is_installed:
            continue
        backend_obj = backend()
        result = backend_obj.linspace(start, stop, steps)
        assert (result == backend_obj.array([0, 10, 20])).all()
...
def test_linspace_2():
    ...
    for backend in all_backend:
        ...
        result = backend_obj.linspace(start, stop, steps, dtype=mithril.int32)
        assert (result == backend_obj.array([0, 10, 20])).all()
