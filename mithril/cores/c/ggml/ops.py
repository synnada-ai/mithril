...
def add(left: PyArray, right: PyArray) -> PyArray:
    return binary_op(left, right, lib.add)


def multiplication(left: PyArray, right: PyArray) -> PyArray:
    return binary_op(left, right, lib.multiplication)
...
def binary_op(
    ...
    right: PyArray,
    op: Callable[
        [ctypes.POINTER(Array), ctypes.POINTER(Array), ctypes.POINTER(Array)], None  # type: ignore
    ],
) -> PyArray:
    ...
    op(ctypes.byref(output.arr), ctypes.byref(left_c.arr), ctypes.byref(right_c.arr))
    _shape = output.shape
