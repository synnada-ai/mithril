...
g_tensor *matrix_multiplication_grad(g_context *ctx, g_tensor *gradient,
  ...
  if (idx == 0) {
    ...
    return ggml_mul_mat(ctx, transpose(ctx, gradient, gradient),
                        transpose(ctx, left, left));
  } else {
...
struct ggml_tensor *squared_error_grad(struct ggml_context *ctx,
                                       struct ggml_tensor *gradient, int idx,
                                       struct ggml_tensor *output,
                                       struct ggml_tensor *left,
                                       struct ggml_tensor *right) {
  struct ggml_tensor *scaled_diff =
      ggml_scale(ctx, ggml_sub(ctx, left, right), 2.0f);
...
struct ggml_tensor *
reduce_mean_grad(struct ggml_context *ctx, struct ggml_tensor *gradient,
  ...
  struct ggml_tensor *scaled_grad =
      ggml_scale(ctx, gradient, 1.0f / (float)size);
