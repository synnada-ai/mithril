...
g_tensor *relu(g_context *ctx, g_tensor *input) { return ggml_relu(ctx, input); }
...
g_tensor *add_grad(g_context *ctx, g_tensor *gradient, int idx, g_tensor *output, g_tensor *left,
                   g_tensor *right) {
  return gradient;
}

g_tensor *multiplication_grad(g_context *ctx, g_tensor *gradient, int idx, g_tensor *output,
                              g_tensor *left, g_tensor *right) {
  if (idx == 0) {
...
g_tensor *transpose_grad(g_context *ctx, g_tensor *gradient, int idx, g_tensor *output,
                         g_tensor *left, g_tensor *right) {
  return transpose(ctx, gradient, gradient);
}

g_tensor *matrix_multiplication_grad(g_context *ctx, g_tensor *gradient, int idx, g_tensor *output,
                                     g_tensor *left, g_tensor *right) {
  if (idx == 0) {
...
g_tensor *relu_grad(g_context *ctx, g_tensor *output_grad, int idx, g_tensor *output,
                    g_tensor *input) {
  return ggml_mul(ctx, ggml_step(ctx, input), output_grad);
}

struct ggml_tensor *squared_error_grad(struct ggml_context *ctx, struct ggml_tensor *gradient,
                                       int idx, struct ggml_tensor *output,
                                       struct ggml_tensor *left, struct ggml_tensor *right) {
  ...
  if (idx == 1) { grad = ggml_neg(ctx, grad); }
  return grad;
  return (idx == 1) ? ggml_neg(ctx, grad) : grad;
}

struct ggml_tensor *reduce_mean_grad(struct ggml_context *ctx, struct ggml_tensor *gradient,
                                     int idx, struct ggml_tensor *left, struct ggml_tensor *right,
                                     struct ggml_tensor *axes, struct ggml_tensor *keepdim) {
