...
int accumulate_grads_helper(struct ggml_tensor *gradient,
                            struct ggml_tensor *input);
struct ggml_tensor *accumulate_grads(struct ggml_context *ctx,
...
struct ggml_tensor *accumulate_grads(struct ggml_context *ctx,
  ...
  if (axes == 0) {
    if (ggml_n_dims(gradient) == 3) {
      struct ggml_tensor *temp = ggml_sum_rows(
          ctx, ggml_cont(ctx, ggml_permute(ctx, gradient, 2, 1, 0, 3)));
      ...
      res = ggml_cont(
          ...
          ggml_transpose(
              ctx, ggml_sum_rows(
                       ctx, ggml_cont(ctx, ggml_transpose(ctx, gradient)))));
    }
    ...
    struct ggml_tensor *temp1 = ggml_cont(
        ctx, ggml_transpose(
                 ctx, ggml_sum_rows(
                          ctx, ggml_cont(ctx, ggml_transpose(ctx, temp)))));
    struct ggml_tensor *temp2 = ggml_sum_rows(
        ctx, ggml_cont(ctx, ggml_permute(ctx, temp1, 2, 1, 0, 3)));
