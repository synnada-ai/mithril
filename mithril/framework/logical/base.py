...
class BaseModel:
    ...
    def _add_connection(
        ...
        if given_connection.connections == set():
            ...
            for idx, conn in enumerate(given_connection.connections):
                ...
                if idx == 0:
                    initial_conn = _conn
                    # TODO: Name should have been updated before this point.
                    # if outer_key is not None:
    ...
    def extract_connection_info(
        ...
        if self.is_primitive:
            ...
            for key in tuple(input_keys) + tuple(self.conns.output_keys):
                ...
                _outer_key = data_to_key_map.get(key_data, [key])
                _outer_key = ["'" + key + "'" for key in _outer_key]
                if key_data.edge_type is not Tensor and key_data.value is not TBD:
                    ...
                    _outer_key = [str(key_data.value)]
                conn.extend(_outer_key)
    ...
    def _set_values(
        self,
        config: Mapping[str | ConnectionData, Tensor[Any] | MainValueType | str]
        | Mapping[ConnectionData, Tensor[Any] | MainValueType | str]
