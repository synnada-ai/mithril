...
class ConnectionData:
    ...
    def _replicate(self) -> Self:
        _con = self.__class__(name=None if self.is_autogenerated else self.key)
        _con.metadata = self.metadata
        return _con
...
class BaseModel:
    ...
    def _add_connection(
        ...
        match given_connection:
            ...
            case ConnectionData() if given_connection.model is None:
        ...
        if is_input:
            if is_new_connection:
                # New input connections are classified wrt their valuedness.
                key_name = given_connection.get_key()
                assert key_name is not None
                key_type = (KeyType.LATENT_INPUT, KeyType.INPUT)[is_not_valued]
            elif self.conns.get_type(con_obj) == KeyType.LATENT_OUTPUT:
                # Latent outputs become internal connections when connected
                # to a connection.
                key_type = KeyType.INTERNAL
        else:
            # Output connections are categorized as latent outputs if they are not
            # connected; otherwise, they are classified as internal (in the case
            # of extending from inputs).
            key_type = (KeyType.LATENT_OUTPUT, KeyType.INTERNAL)[
                con_obj in self.conns.input_connections
            ]
        # If needed, set connection type.
        if key_type:
    ...
    def _rename_key(
        ...
        if check_key and key in self.conns.all:
            raise KeyError(f"Key '{key}' is already used!")
    ...
    def rename_key(self, connection: ConnectionData, key: str) -> None:
        self._rename_key(connection, key)
    ...
    def merge_connections(
        ...
        if name is not None:
            # Check provided name is not already used in the model
            # other than the connections to be merged.
            conn_names = {
                self.conns.get_extracted_connection(conn).key
                if isinstance(conn, ConnectionData)
                else conn
                for conn in connections
            }
            if name in (self.conns.all.keys() - conn_names):
                raise KeyError(
                    f"Key '{name}' is already used in the model. "
                    "Please provide a unique name for the merged connection."
                )
    ...
    def _merge_connections(
        ...
        if name is None:
            if not conn1.is_autogenerated and not conn2.is_autogenerated:
                ...
                name = conn2.key
        if name is not None:
            self._rename_key(conn1, name, False)
    ...
    def _extend_with_submodels(
        ...
        """
        Extend the current model with the submodels of a given model.

        This method is primarily used when you want to extend submodels rather than
        directly extending the model. It iterates over each submodel (by default
        retrieved from model.dag if none are provided) and updates its connections.
        For each connection in the submodel, it checks against the main model's
        connections to ensure consistency, replicating the connection if needed along
        with handling autogenerated connections or exposing keys, before finally
        extending the self model with the updated submodel.

        Parameters:
            model (BaseModel): The base model whose submodels are to be extended.
            submodels (Iterable[BaseModel] | None, optional): A collection of submodels
                to be extended. If None, the submodels are obtained from model.dag.
            clear (bool, optional): When False, replicates connections before
                processing. When True, does not replicate and clears the model
                Default is True.
        ...
        for sub_m in submodels:
            ...
            for con in sub_m.conns.all.values():
                ...
                if _con is None:
                    continue
                autogenerated = _con.is_autogenerated
                if not clear:
                    _con = _con._replicate()

                if (
                    not autogenerated
                    and self.conns.get_con_by_metadata(con.metadata) is None
                ):
                    # If the connection is not autogenerated, we need to check if the
                    # connection key is already in the main model.
                    self_con = self.conns.get_connection(_con.key)
                    if self_con is not None:
                        model.conns.connections_dict.setdefault(
                            self_con.metadata, set()
                        )
                    ...
                    self.conns.get_con_by_metadata(con.metadata) is None
                    and autogenerated
                    and model.conns.get_type(_con)
                    in {KeyType.LATENT_OUTPUT, KeyType.INTERNAL}
                ):
                    _con.model = None
                    _con.set_key(None)
                    conns[con.key] = _con
                ...
                if clear:
                    # Remove the connections object from
                    # the submodel's connections_dict.
                    sub_m.conns.connections_dict[edge].discard(model.conns)
    ...
    def get_unique_submodel_names(self) -> dict[BaseModel, str]:
        ...
        for model in self.dag:
            if model.name:
                ...
                model_type_dict.setdefault(model.default_name, []).append(model)
