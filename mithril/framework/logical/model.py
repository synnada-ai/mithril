    ...
    for c in connections:
        if (
            ...
            if (
                ...
                key = None
                if not _c.is_autogenerated:
                    key = _c.key
                con = provisional_model._create_connection(_c.metadata, key)
            _connections.append(con)
        else:
            _connections.append(c)

    # Add all new models to provisional_model.
    for m, m_c in new_models.items():
        ...
        if m.provisional_source:
            assert isinstance(m, Model)
            provisional_model.extend_extracted_model(m, m_c)
        else:
...
class Connection(ConnectionData):
    ...
    def __neg__(self) -> Connection:
        return create_extracted_model(connections=[self], model=NegateOp)
...
class Model(BaseModel):
    ...
    def _unroll_template(
        ...
        # extend self with submodels of provisional model.
            ...
            # elif p_model is True:
            #     if self.provisional_model is False:
            #         self._bind_provisional_model(extract_m)
            #     else:
            #         assert isinstance(self.provisional_model, Model)
    ...
    def extend_extracted_model(self, model: Model, start_con: ConnectionData) -> None:
        ...
        for sub_m in model.get_models_in_topological_order(_start_m):
            if sub_m not in self.dag:
                self._extract_submodel(model, sub_m)
            sub_models.add(sub_m)

        # Merge provisional models
        if model.provisional_source is True:
