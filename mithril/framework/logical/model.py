...
class Model(BaseModel):
    ...
    def _set_value(self, key: ConnectionData, value: MainValueType | str) -> Updates:
        if isinstance(key.metadata.data, Tensor):
            if key.conn.key not in self.conns._connection_dict[KeyType.INPUT]:
                raise KeyError(f"Given key: '{key.conn.key}' is not an input key!")

            extend_value: MainValueType | IOKey | str = value
            # If ToTensor output key is reserved key, rename it.
            if not key.conn.key.startswith("$"):
                key_name = key.conn.key
                self.inter_key_count += 1
                ...
                key.is_key_autogenerated = True
                extend_value = IOKey(key_name, value)
    ...
    def handle_auto_conversion(
        ...
        # ):
            ...
            if (isinstance(key_data, Scalar)) and (connection._shape is not None):
            ...
            elif isinstance(key_data, Scalar):
                if set_type is None:
            ...
            return self.handle_auto_conversion(
                key_data, is_input, new_conn, updates=updates
            )
            ...
            if isinstance(connection.key, IOKey):
                ...
                if output_connection is not None:
                    data = output_connection.metadata.data
                    is_conn_input = False

                elif connection.key._value is not TBD:
                    data = Scalar(value=connection.key._value)
                    is_conn_input = True
                    ...
                    data = (
                        next(iter(connections)).metadata.data if types else key_data
                ...
                base_conn = self.handle_auto_conversion(
                    data, is_conn_input, connection.key, updates
                )
        ...
        return self.create_connection_model(
            connection_type, key_data, is_input, connection
        )
    ...
    def create_connection_model(
        ...
        if connection_type is not None:
            is_source = is_input and key_data.value is TBD
            coercion_type = (
                (connection_type, key_data.__class__)
                if is_source
                else (key_data.__class__, connection_type)
            )
    ...
    def extend(
        ...
        **kwargs: ConnectionType,
        ...
        for key, value in kwargs.items():
            ...
            inner_data = model.conns.all[key].metadata.data
            if (
                updated_conn := self.handle_auto_conversion(
                    inner_data, is_input, kwargs[key], updates
                )
            ) is not None:
